...
rm -f tmp; make clean > /dev/null 2>&1

h=$(grep -Eo '[[:xdigit:]]{40} +-$' \
         uniq_fn.txt) && h=${h::40} || exit 1
func="func_${h::11}"
sed -Ei "s/($h +)-$/\1$func/" uniq_fn.txt &&
git add uniq_fn.txt || exit 1

f=($(grep -h "^$h" fn_norm.txt | cut -c49- | cut -d. -f1))
...
mv tmp layer.h && git add layer.h         &&

{ echo -n "$hdr" | xargs git rm; } &&
make -j32 model.c.o activation.c.o &&
git commit -m "$func()"
